...
<div class="space-y-6">
  <div class="flex justify-between items-center">
    ...
    <button class="btn btn-primary" on:click={loadOldFiles} disabled={isLoading}>
      {#if isLoading}
      ...
      <div class="form-control">
        <label class="label">
          ...
          <span class="label-text-alt font-bold"
            >{threshold} days ({(threshold / 30).toFixed(1)} months)</span
          >
        </label>
        <input
          type="range"
    ...
    <div class="stats shadow w-full">
      ...
      <div class="stat">
        ...
        <div class="stat-value text-info">
          {allFiles.length > 0
            ? formatAge(allFiles.reduce((sum, f) => sum + (f.age_days || 0), 0) / allFiles.length)
            : "N/A"}
        </div>
        ...
        <div class="flex justify-between items-center mb-4">
          ...
          <div class="flex gap-2">
            <button class="btn btn-sm btn-outline" on:click={toggleAll}>
              {oldFiles.every((file) => selectedFiles.has(file.path))
                ? "Deselect Page"
                : "Select Page"}
            </button>
            <button
